class Animacao {
  ...
  excluirProcessamento (processamento) {
    this.processamentos.forEach((p, index) => {
      if (p === processamento) { this.processamentos.splice(index, 1) }
    })
  }
  ...
  mensagem (msg) {
    this.context.save()
    this.context.font = '55px palatino'
    this.context.fillStyle = 'white'
    this.context.strokeStyle = 'rgba(100,50,50,0.4)'
    this.context.textAlign = 'center'
    this.context.textBaseline = 'middle'
    const w = this.context.canvas.width / 2
    const h = this.context.canvas.height / 3

    this.context.fillText(msg, w, h)
    this.context.strokeText(msg, w, h)
